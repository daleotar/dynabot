...
QDynScene::QDynScene(QWidget *parent) : QGLWidget(parent)
{
    xRot = -45.0;
    yRot = 0;
    zRot = -45.0;
...
QDynScene::~QDynScene()
{
    ...
    {
        delete Robots.takeAt(i);
    }
    for(int i=(nObstacles-1); i>=0; i--)
    {
        delete Obstacles.takeAt(i);
    }
}

void QDynScene::initializeGL() {
...
void QDynScene::resizeGL(int width, int height)
{
    ...
    glLoadIdentity();
    gluPerspective(angle, aspect, fovy, 500.0);
    glMatrixMode(GL_MODELVIEW);
    glLoadIdentity();
}

void QDynScene::paintGL()
{
...
//        glDisable(GL_LINE_STIPPLE);
    ...
    {
        f_p_s_ = 1000.0 * (float)maxCounter / (float)fpsTime_.restart();
//        fpsString_ = QString("%1Hz").arg(f_p_s_, 0, 'f', ((f_p_s_ < 10.0)?1:0));
        fpsString_ = QString::number(f_p_s_) + "Hz";
        fpsCounter_ = 0;
    }
...
void QDynScene::drawCoordinates(double x, double y, double z)
{
    glDisable(GL_LIGHTING);
    glEnable(GL_LINE_STIPPLE);
...
void QDynScene::drawBox(double xMin, double xMax, double yMin, double yMax, double zMin, double zMax)
{
    glDisable(GL_LIGHTING);
    glEnable(GL_LINE_STIPPLE);
    glLineStipple(3,0xAAAA);
...
void QDynScene::drawArrow(float length, int nbSubdivisions)
{
...
void QDynScene::drawAxis(double length, bool opaque)
{
...
void QDynScene::drawGrid(float size, int nbSubdivisions)
{
    glDisable(GL_LIGHTING);
    glBegin(GL_LINES);
...
void QDynScene::drawMesh(QDynMesh *mesh, bool isColliding, int type)
{
...
void QDynScene::mousePressEvent(QMouseEvent *event)
{
    lastMousePos = event->pos();
}

void QDynScene::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if (event->buttons() & Qt::LeftButton)
    {
        xRot += dy/3.5;
        zRot += dx/2.5;
        xRot = QDynMath::normalizeAngle(xRot);
        zRot = QDynMath::normalizeAngle(zRot);
        this->updateGL();
...
void QDynScene::wheelEvent(QWheelEvent *event)
{
...
void QDynScene::addRobot(QString filename)
{
    Robots << new QDynRobot(filename,this->parent());
    RobotsProcesses << new QDynRobotThread(this->parent());
...
void QDynScene::setRobotLinkQ(double q, int robot, int link, bool refreshBounds)
{
    Robots[robot]->setQ(q,link,false,refreshBounds);
    checkCollisions();
}

void QDynScene::setRobotProp(int prop, QVariant value, int robot)
{
    Robots[robot]->setProp(prop,value);
    checkCollisions();
}

void QDynScene::resetRobot(int index)
{
    Robots[index]->resetRobot();
    checkCollisions();
}

void QDynScene::deleteRobot(int index)
{
...
void QDynScene::addRobotPath(int robotIndex, QDynPath *path, int linkIndex, double appSpeed, double speed)
{
    Robots[robotIndex]->addPath(path,linkIndex,appSpeed,speed);
}

void QDynScene::deleteRobotPath(int robotIndex, int pathIndex)
{
    Robots[robotIndex]->deletePath(pathIndex);
}

void QDynScene::setInitialPathConfig(int robotIndex)
{
...
void QDynScene::startSimulation()
{
...
void QDynScene::pauseSimulation()
{
...
void QDynScene::resumeSimulation()
{
    // Pause all robot threads and emit the signal.
    for(int i=0; i<RobotsProcesses.length(); i++)
    {
...
void QDynScene::robotProcessFinished()
{
    QDynRobotThread *robotProcess = static_cast<QDynRobotThread*>(sender());
...
void QDynScene::robotProcessUpdated(int procType, int percentage)
{
    if(procType == 1)
    {
...
void QDynScene::addObstacle(QString filename)
{
    Obstacles << new QDynObstacle(filename);
    checkCollisions();
}

void QDynScene::setObstacleProperties(QString newname, double tx, double ty, double tz, double rx, double ry, double rz, int index)
{
    Obstacles[index]->setProperties(newname, tx, ty, tz, rx, ry, rz);
    checkCollisions();
}

void QDynScene::setObstacleProperties(QDynObstacle::QDynLocRotProps propertie, QVariant value, int index)
{
    if(propertie == QDynObstacle::PropName)
    {
...
void QDynScene::deleteObstacle(int index)
{
...
void QDynScene::addPath(QString filename)
{
    Paths << new QDynPath(filename);
    updateGL();
}

void QDynScene::addPath(double xIni,double yIni, double zIni, double xFin, double yFin, double zFin, int n)
{
    Paths << new QDynPath(xIni,yIni,zIni,xFin,yFin,zFin,n);
    updateGL();
}

void QDynScene::setPathProperties(int prop, QVariant value, int index)
{
...
void QDynScene::deletePath(int index)
{
    for(int i=0; i<Robots.length(); i++)
    {
...
void QDynScene::toggleBlend(bool state)
{
    enabledBlend = state;
...
void QDynScene::toggleGlobalAxis(bool state)
{
    enabledGlobalAxis = state;
    updateGL();
}

void QDynScene::toggleLocalAxes(bool state)
{
    enabledLocalAxes = state;
    updateGL();
}

void QDynScene::toggleGrid(bool state)
{
    enabledGrid = state;
    updateGL();
}

void QDynScene::toggleObjectCoordinates(bool state)
{
...
void QDynScene::checkCollisions()
{
    int nObstacles = Obstacles.length();
    int nRobots = Robots.length();
...
void QDynScene::setCollisionType(QDynTypeCollision type)
{
    collisionType = type;
    checkCollisions();
}
