...
QDynLink::QDynLink(QString meshfile, QString linkName)
{
    mesh = new QDynMesh(meshfile);
    ...
    // Rotation Matrices
    R0a = new QDynMatrix(true,3);
    R0b = new QDynMatrix(true,3);
    Rab = new QDynMatrix(true,3);
    Ra0 = new QDynMatrix(true,3);
    Rb0 = new QDynMatrix(true,3);
    Rba = new QDynMatrix(true,3);

    // Physical properties of the Link
    I   = new QDynMatrix(true,3);
    s   = new QDynMatrix(3,1);
    p   = new QDynMatrix(3,1);
    mass = 1;
    density = 1000;

    // Dynamical System Matrices
    Asys = QDynGeneric::matrixFromString("[-1 -1;1 0]");
    Bsys = QDynGeneric::matrixFromString("[1;0]");
    Csys = QDynGeneric::matrixFromString("[0 1]");
    Dsys = QDynGeneric::matrixFromString("[0]");

    updateP();
//    setBaseFrame(T0a);
    QDynMath::setTransIdentity(T0a);
    QDynMath::setTransIdentity(Tab);
    QDynMath::setTransIdentity(T0b);
}
...
void QDynLink::computeInertiaAndCenterOfMass()
{
...
void QDynLink::updateDH()
{
    QDynMath::convDH2Trans(a,alpha,d,theta + plusTheta,Tab);
}

void QDynLink::updateP()
{
    p->data[0][0] = a;
    p->data[1][0] = d * sin(QDynMath::deg2Rad(alpha));
    p->data[2][0] = d * cos(QDynMath::deg2Rad(alpha));
}

void QDynLink::setBaseFrame(double T[4][4], bool secondaryMatrices)
{
    QDynMath::copyMatTrans(T,T0a);
    updateDH();
    QDynMath::multTransMat(T0a,Tab,T0b);
    QDynMath::convTrans2RotTra(T0b,RLink,PLink);
    QDynMath::convRot2RPY(RLink,roll,pitch,yaw);

    if(secondaryMatrices == true)
    {
        QDynMath::copyRotFromTrans(T0a,R0a);
        QDynMath::copyRotFromTrans(T0b,R0b);
        QDynMath::copyRotFromTrans(Tab,Rab);
...
void::QDynLink::updateglMatrices()
{
    QDynMath::convTrans2glMat(T0b,glMat);
}
...
void QDynLink::printLinkFeatures()
{
//    qDebug() << "a =" << a << ", alpha =" << alpha << ", d =" << d << ", theta =" << theta;
//    qDebug() << "Ta =";
//    QDynMath::printTransMat(T0a);
//    qDebug() << "Tb =";
//    QDynMath::printTransMat(T0b);
//    qDebug() << "DH =";
//    QDynMath::printTransMat(Tab);
}
